...
//WebGL spec GL externs
    ...
    inline static function vertexAttrib1f(index:GLuint, x:Float):Void
        GL.glVertexAttrib1f(index, x);

    inline static function vertexAttrib2f(index:GLuint, x:Float, y:Float):Void
        GL.glVertexAttrib2f(index, x, y);

    inline static function vertexAttrib3f(index:GLuint, x:Float, y:Float, z:Float):Void
        GL.glVertexAttrib3f(index, x, y, z);

    inline static function vertexAttrib4f(index:GLuint, x:Float, y:Float, z:Float, w:Float):Void
    ...
    static inline function bufferData(target:GLenum, data:ArrayBufferView, usage:GLenum):Void {
        force_include;
        untyped __cpp__('glBufferData({0}, {1}, (GLvoid*)(&{2}->bytes->b[0] + {2}->byteOffset), {3})', target, data.byteLength, data, usage);
    }

    static inline function bufferSubData(target:GLenum, offset:Int, data:ArrayBufferView):Void {
        force_include;
        untyped __cpp__('glBufferSubData({0}, {1}, {2}, (GLvoid*)(&{3}->bytes->b[0] + {3}->byteOffset))', target, offset, data.byteLength, data);
    }
    ...
    static inline function readPixels(x:Int, y:Int, width:Int, height:Int, format:GLenum, type:GLenum, data:ArrayBufferView):Void {
        force_include;
        untyped __cpp__('glReadPixels({0}, {1}, {2}, {3}, {4}, {5}, (GLvoid*)(&{6}->bytes->b[0] + {6}->byteOffset))', x, y, width, height, format, type, data.byteLength, data);
    }
    ...
    static inline function texImage2D(target:GLenum, level:Int, internalformat:GLenum, width:Int, height:Int, border:Int, format:GLenum, type:GLenum, data:ArrayBufferView):Void {
        force_include;
        untyped __cpp__('GLvoid* __linc_opengl_data = (({0} != null()) ? (GLvoid*)(&{0}->bytes->b[0] + {0}->byteOffset) : NULL)', data);
        untyped __cpp__('glTexImage2D({0}, {1}, {2}, {3}, {4}, {5}, {6}, {7}, __linc_opengl_data)', target, level, internalformat, width, height, border, format, type);
    }
    ...
    static inline function texSubImage2D(target:GLenum, level:Int, xoffset:Int, yoffset:Int, width:Int, height:Int, format:GLenum, type:GLenum, data:ArrayBufferView):Void {
        force_include;
        untyped __cpp__('glTexSubImage2D({0}, {1}, {2}, {3}, {4}, {5}, {6}, {7}, (GLvoid*)(&{8}->bytes->b[0] + {8}->byteOffset))', target, level, xoffset, yoffset, width, height, format, type, data);
    }

    static inline function uniform1fv(location:GLUniformLocation, data:Float32Array):Void {
        force_include;
        untyped __cpp__('glUniform1fv({0}, {1}, (GLfloat*)(&{2}->bytes->b[0] + {2}->byteOffset))', location.id, data.view.byteLength / 4, data.view);
    }

    static inline function uniform1iv(location:GLUniformLocation, data:Int32Array):Void {
        force_include;
        untyped __cpp__('glUniform1iv({0}, {1}, (GLint*)(&{2}->bytes->b[0] + {2}->byteOffset))', location.id, data.view.byteLength / 4, data.view);
    }

    static inline function uniform2fv(location:GLUniformLocation, data:Float32Array):Void {
        force_include;
        untyped __cpp__('glUniform2fv({0}, {1}, (GLfloat*)(&{2}->bytes->b[0] + {2}->byteOffset))', location.id, data.view.byteLength / 8, data.view);
    }

    static inline function uniform2iv(location:GLUniformLocation, data:Int32Array):Void {
        force_include;
        untyped __cpp__('glUniform2iv({0}, {1}, (GLint*)(&{2}->bytes->b[0] + {2}->byteOffset))', location.id, data.view.byteLength / 8, data.view);
    }

    static inline function uniform3fv(location:GLUniformLocation, data:Float32Array):Void {
        force_include;
        untyped __cpp__('glUniform3fv({0}, {1}, (GLfloat*)(&{2}->bytes->b[0] + {2}->byteOffset))', location.id, data.view.byteLength / 12, data.view);
    }

    static inline function uniform3iv(location:GLUniformLocation, data:Int32Array):Void {
        force_include;
        untyped __cpp__('glUniform3iv({0}, {1}, (GLint*)(&{2}->bytes->b[0] + {2}->byteOffset))', location.id, data.view.byteLength / 12, data.view);
    }

    static inline function uniform4fv(location:GLUniformLocation, data:Float32Array):Void {
        force_include;
        untyped __cpp__('glUniform4fv({0}, {1}, (GLfloat*)(&{2}->bytes->b[0] + {2}->byteOffset))', location.id, data.view.byteLength / 16, data.view);
    }

    static inline function uniform4iv(location:GLUniformLocation, data:Int32Array):Void {
        force_include;
        untyped __cpp__('glUniform4iv({0}, {1}, (GLint*)(&{2}->bytes->b[0] + {2}->byteOffset))', location.id, data.view.byteLength / 16, data.view);
    }

    static inline function uniformMatrix2fv(location:GLUniformLocation, transpose:Bool, data:Float32Array):Void {
        force_include;
        untyped __cpp__('glUniformMatrix2fv({0}, {1}, {2}, (GLfloat*)(&{3}->bytes->b[0] + {3}->byteOffset))', location.id, data.length >> 2, transpose, data.view);
    }

    static inline function uniformMatrix3fv(location:GLUniformLocation, transpose:Bool, data:Float32Array):Void {
        force_include;
        untyped __cpp__('glUniformMatrix3fv({0}, {1}, {2}, (GLfloat*)(&{3}->bytes->b[0] + {3}->byteOffset))', location.id, data.length / 9, transpose, data.view);
    }

    static inline function uniformMatrix4fv(location:GLUniformLocation, transpose:Bool, data:Float32Array):Void {
        force_include;
        untyped __cpp__('glUniformMatrix4fv({0}, {1}, {2}, (GLfloat*)(&{3}->bytes->b[0] + {3}->byteOffset))', location.id, data.length >> 4, transpose, data.view);
    }

    static inline function vertexAttrib1fv(index:GLuint, data:Float32Array):Void {
        force_include;
        untyped __cpp__('glVertexAttrib1fv({0}, (GLfloat*)(&{1}->bytes->b[0] + {1}->byteOffset))', index, data.view);
    }

    static inline function vertexAttrib2fv(index:GLuint, data:Float32Array):Void {
        force_include;
        untyped __cpp__('glVertexAttrib2fv({0}, (GLfloat*)(&{1}->bytes->b[0] + {1}->byteOffset))', index, data.view);
    }

    static inline function vertexAttrib3fv(index:GLuint, data:Float32Array):Void {
        force_include;
        untyped __cpp__('glVertexAttrib3fv({0}, (GLfloat*)(&{1}->bytes->b[0] + {1}->byteOffset))', index, data.view);
    }

    static inline function vertexAttrib4fv(index:GLuint, data:Float32Array):Void {
        force_include;
        untyped __cpp__('glVertexAttrib4fv({0}, (GLfloat*)(&{1}->bytes->b[0] + {1}->byteOffset))', index, data.view);
    }

    static inline function vertexAttribPointer(index:GLuint, size:Int, type:GLenum, normalized:Bool, stride:Int, offset:Int):Void {
        force_include;
        untyped __cpp__('glVertexAttribPointer({0}, {1}, {2}, {3}, {4}, (void *)(intptr_t){5})', index, size, type, normalized, stride, offset);
    }
